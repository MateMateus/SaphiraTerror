...
using SaphiraTerror.Application.DTOs;                            // DTOs expostos pela camada Application (FilmeDto, PagedResult<T>).
using SaphiraTerror.Application.Filters;                         // Filtro de consulta/paginação (FilmeFilter).
using SaphiraTerror.Application.Services;                        // Serviço de aplicação para consultas (IFilmeQueryService).
using SaphiraTerror.Application.Abstractions.Repositories;       // Abstração de repositório (IFilmeRepository) para busca por Id.
...
public class FilmesController : ControllerBase
{
    ...
    public async Task<ActionResult<FilmeDto>> GetById(int id, CancellationToken ct)
    {
        ...
        var dto = new FilmeDto(                                 // Projeta entidade em DTO (não vaza domínio para a UI).
            ...
            f.Classificacao.Nome                                // Nome da classificação (navegação carregada pelo repo).
        );
